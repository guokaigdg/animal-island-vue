...
Props: default slot (VNode tree) or `text` prop, speed=90ms, trigger (any unknown; change to restart),
       autoPlay=true, emits `done` event
Behavior: recursively truncates the VNode tree (built via `h()` or template) by character count
          while preserving element structure, class, and inline styles. Returns a plain fragment
          (NO extra wrapping div/span) so it has ZERO layout impact.
Usage example (Composition API, render via `h()`):
  import { h } from 'vue'
  const tree = h('span', { class: 'note' }, [
    'Welcome to ',
    h('strong', 'animal-island-vue'),
    '!'
  ])
  // <Typewriter :speed="90" @done="onDone">{{ tree }}</Typewriter>
Or template form:
  <Typewriter :speed="90" @done="onDone">
...
Token colors:
  ...
  jsx       #f0a870 (`<Tag`, `</Tag`, `/>` — template tag delimiters)
