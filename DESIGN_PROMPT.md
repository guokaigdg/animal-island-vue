...
Props: speed=90ms, trigger (any unknown; change to restart), autoPlay=true, text? (string shortcut)
Slot: default (any VNode tree). Emits: done.
Behavior: recursively truncates VNode tree by character count while preserving
          element structure, class, and inline styles. Returns a plain fragment
          (NO extra wrapping div/span) so it has ZERO layout impact.

=== COMPONENT INVENTORY (19 exports from src/index.ts) ===
...
Utility:     Loading, Table   (Vue-only additions)
...
Token colors:
  ...
  component #80c0e0 (PascalCase identifiers — JSX/SFC tags / type names)
...
- 项目使用 **Less + CSS Custom Properties**（参见 `src/styles/variables.less` 与 `src/styles/themes/default.less`），所有 token 暴露为 CSS 自定义属性：`--animal-primary-color` / `--animal-spacing-*` / `--animal-border-radius-*` / `--animal-shadow-*` 等，可在 `:root` 上运行时覆盖换肤。
- 单文件组件 SFC + `<script setup>` + 泛型；`Table` 使用 `generic="T extends TableRecord"`。
- 组件 API 适配 Vue 习惯：`v-model` / `v-model:open` / `v-model:expanded`、命名插槽（`#title` / `#prefix` / `#footer` / `#checked` 等）取代 React 的 `ReactNode` props。
- React 端的 17 个组件 + Vue 新增 `Loading` / `Table` = **19 个组件**。
- 对外发布：`animal-island-vue` 包含 `dist/` 与 `dist/index.css`，使用 `import 'animal-island-vue/style'` 一次性引入样式。
